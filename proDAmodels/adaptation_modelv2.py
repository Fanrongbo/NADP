...
from proDAmodels.sync_batchnorm import SynchronizedBatchNorm2d, DataParallelWithCallback
from proDAmodels.deeplabv2 import Deeplab
from proDAmodels.discriminator import FCDiscriminator
from .utils import freeze_bn, get_scheduler, cross_entropy2d
from proDAdata.randaugment import affine_sample
...
class CustomModel():
    def __init__(self, opt):
        ...
        self.nets = []
        self.nets_DP = []
        self.default_gpu = opt.default_gpu
        ...
        if self.opt.stage == 'warm_up':
            ...
            self.net_D_DP = self.init_device(self.net_D, gpu_id=self.default_gpu, whether_DP=False)
    ...
    def step_adv(self, source_x, source_label, target_x, source_imageS, source_params):
        ...
        # self.imageo=
        self.source_outputUpout=source_outputUp
        # self.lableo1=source_labelS
        self.lableo2 = source_label
        #目标域GAN
    ...
    def step(self, source_x, source_label, target_x, target_imageS=None, target_params=None, target_lp=None, 
        ...
        self.source_outputUpout=source_outputUp
        loss_GTA = cross_entropy2d(input=source_outputUp, target=source_label)#source CE
        loss_GTA.backward()        
        ...
        target_out = self.BaseNet_DP(target_imageS) if self.opt.S_pseudo > 0 else self.BaseNet_DP(target_x)#target
        ...
        if self.opt.proto_rectify:
            ...
            argmax = rectified.max(1, keepdim=True)[0]#归一化矫正后的概率分布
            threshold_arg[argmax < self.opt.train_thred] = 250#筛选置信度低于阈值的预测
        ...
        if self.opt.proto_consistW > 0:
            ...
            ema2weak_feat_proto_distance = self.feat_prototype_distance(ema2weak_feat)  #N*19*H*W 质心和特征的距离
        ...
        if self.opt.moving_prototype: #update prototype
            ...
            for t in range(len(ema_ids)):
                self.update_objective_SingleVector(ema_ids[t], ema_vectors[t].detach(), start_mean=False)#更新质心
