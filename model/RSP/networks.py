...
class Backbone(torch.nn.Module):
    def __init__(self, args, input_nc, output_nc,
        ...
        self.classifier = TwoLayerConv2d(in_channels=32, out_channels=7)
...
class BASE_Transformer(Backbone):
    ...
    def forward(self, x):
        ...
        if self.token_trans:
            self.tokens = self._forward_transformer(token)
            # token1, token2 = self.tokens.chunk(2, dim=1)
        # forward transformer decoder
        if self.with_decoder:
            x = self._forward_transformer_decoder(x, self.tokens)
        else:
            x = self._forward_simple_decoder(x, self.tokens)
