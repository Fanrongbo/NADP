...
class Vailidate():
    ...
    def main(self, model, figure_metrics=None, epoch=1, cfg=None, iterFlag=False, dataloader=None, currentOpt=None,ProtoInput=None,DomainLabel=0):
        ...
        with torch.no_grad():
            for i in tbar:
                ...
                seg_val_pred, _, _ = model.forward(data_img_val, DomainLabel=DomainLabel,ProtoInput=ProtoInput)
...
class VailidateSim():
    def __init__(self, opt, device=None,Osize=32,n_cluster=1,CatFlag=False):
        ...
        self.CatFlag=CatFlag
        self.figure_metrics = self.metrics.initialize_figure()
    def main(self, model, figure_metrics=None, epoch=1, cfg=None, iterFlag=False, dataloader=None, currentOpt=None,ProtoInput=None,classifierFlag=False,DomainLabel=0):
        val_dataload = dataloader.load_data()
        iter_val = iter(val_dataload)
        val_data_len = len(val_dataload)
        epoch_start_time = time.time()
        ...
        with torch.no_grad():
            for i in tbar:
                ...
                data_img_val = Variable(data_val['img_full']).to(self.DEVICE)
                label = Variable(data_val['label_full']).to(self.DEVICE)
                seg_val_pred, _, Feat = model.forward(data_img_val,DomainLabel=DomainLabel,ProtoInput=ProtoInput)
                # Generation: source
                label = F.interpolate(label.unsqueeze(1).float(), size=(self.Osize, self.Osize), mode='nearest')
                label = label.squeeze(1)  # 尺寸变为 [14, 512, 512]
                if classifierFlag:
...
class ProtoDistTest():
    ...
    def main(self,model,inputProto=None,epoch=1,cfg=None,iterFlag=False,dataloader=None,currentOpt=None,DomainLabel=0):
        ...
        with torch.no_grad():
            for i in tbar:
                ...
                data_img_val = Variable(data_val['img_full']).to(self.DEVICE)
                label = Variable(data_val['label_full']).to(self.DEVICE)
                seg_val_pred, PrototypeT, FeatTT = model.forward(data_img_val,DomainLabel=DomainLabel,ProtoInput=inputProto)
                # Generation: source
